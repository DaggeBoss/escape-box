...
const conceptAccessRoutes = require('./routes/conceptAccess');
...
// Health check
app.get('/', (req, res) => {
  res.json({ status: 'ok', service: 'gamemaster-backend', version: '2.2.0' });
});
...
app.use('/api/concept-access', conceptAccessRoutes);
// MIDLERTIDIG alias slik at dagens frontend fungerer til frontend-sesjonen er gjort
app.use('/api/scenarios', conceptRoutes);
...
initDatabase()
  .then(() => {
    server.listen(PORT, () => {
      console.log(`🚀 GameMaster backend kjører på port ${PORT}`);
