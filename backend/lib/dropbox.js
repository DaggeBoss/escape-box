...
// Basert på BME Portal-modulen, med:
//   - egen rotmappe (/Escape Box/)
//   - permanente shared links per fil
//   - sti-byggere for scenario-bilder (kort + minispill)
...
async function uploadFile(fileBuffer, dropboxPath, overwrite = false) {
  ...
  // 401 = token er ugyldig. Refresh og prøv på nytt én gang.
...
// Etter sesjon 1-opprydding er det kun 'cards' og 'minigames' som er gyldige.
// 'cards' = bilder brukt på trigger-kort i passord-systemet.
// 'minigames' = ikoner/thumbnails for minispill (selve HTML-filene lastes
// opp som dokumenter med egen flyt i sesjon 3).
const VALID_IMAGE_KINDS = ['cards', 'minigames'];

function buildScenarioImagePath(scenarioId, kind, originalFilename) {
  if (!VALID_IMAGE_KINDS.includes(kind)) {
    throw new Error(`Ugyldig bilde-kind: ${kind}. Må være en av ${VALID_IMAGE_KINDS.join(', ')}`);
  }

  // For ad-hoc upload: legg på timestamp + random for å unngå kollisjoner.
  // Vi har ikke lenger deterministiske PNG-eksport-filnavn (Grid/Bunke/Block)
  // siden investigation board er fjernet — alle bilder er nå brukeropplastede.
